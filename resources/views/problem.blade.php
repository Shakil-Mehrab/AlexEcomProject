debugbar not install
Auth\RegisterController@action not work
server double entry
...
class CreateProductVariationStockView extends Migration
{
    ...
    public function up()
    {
        DB::statement("
            ...
            COLAESCE(SUM(stocks.quantity)-COLAESCE(SUM(product_variation_order.quantity), 0), 0) as stock,
            case when COLESCE(SUM(stocks.quantity)-COLAESCE(SUM(product_variation_order.quantity), 0), 0) > 0
            then true
            else false
            end in_stock
